...
const Dropdown = ({ options }) => {
    ...
    return (
        ...
        <div className="dropdown">
            ...
            <div onMouseOver={() => setDisplay(true)} className="dropdown-menu">
                <span>Select an option</span>
                <i className="fa-solid fa-caret-down"></i>
            </div>

            {/* Dropdown items is displayed only when display usestate is true */}
            {/* Items are comming from the parent components as props */}
            {display ? <ul className="menu-items">{menuItems}</ul> : ""}

            {/* For the first time when the user enters the site then selected is blank so noting is displayed */}
